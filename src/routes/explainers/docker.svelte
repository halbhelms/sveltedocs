...
<main class="mx-8">
  <p class="mb-4">Does Docker run on Linux, macOS, and Windows? You can run both Linux and Windows programs and executables in Docker containers. The Docker platform runs natively on Linux (on x86-64, ARM and many other CPU architectures) and on Windows (x86-64) and works with <SeeMoreLink linkedText="<code>+page.server.js</code>" on:change_see_more seeMoreID=2/>.</p>

  <p class="mb-4">Docker Inc. builds products that let you build and run using containers on Linux, Windows and macOS. What does Docker technology add to just plain LXC? Docker technology is not a replacement for LXCopen_in_new. "LXC" refers to capabilities of the Linux kernel (specifically namespaces and control groups) which allow sandboxing processes from one another, and controlling their resource allocations. On top of this low-level foundation of kernel features, Docker offers a high-level tool with several powerful functionalities:</p>

  <ul class="list-disc grid gap-4 ml-8">
